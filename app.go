...
func (a *App) Connect(name string) error {
	...
	if err != nil {
		zap.L().Warn("Could not get password", zap.String("server", name), zap.Error(err))
		password = ""
	}

	zap.L().Debug("Connecting to server", zap.String("server", name), zap.String("host", server.Host), zap.String("user", server.User))
	...
	if password != "" {
		...
		zap.L().Debug("Using sshpass for authentication")
	} else {
		cmdStr = targetCmd
		zap.L().Debug("No password provided, using standard ssh")
	}
